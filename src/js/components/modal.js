...
export const createModal = (parent, child, classNames) => {
  const modalContainer = createDiv({
    parent: document.querySelector(".main"),
    classNames: ["modal", ...classNames],
  });

  initialPaintForce(modalContainer, "show");
  const modalClose = createSpan({
    ...
    events: {
      click: () => {
        ...
        attachEvent(modalContainer, "transitionend", () =>
  ...
  appendElement(child, modalContainer);
};
